...
/**
 * Class to represent a ttek PDO LTI Data Connector
 *
 * Class LTI_Data_Connector_Tek
 */
class LTI_Data_Connector_Tek extends LTI_Data_Connector
{
    ...
    public function Tool_Consumer_load($consumer)
    {
        ...
        $sql = 'SELECT name, secret, ltiVersion, consumerName, consumerVersion, consumerGuid, cssPath, protected, enabled, enableFrom, enableTo, lastAccess, created, modified ' .
        ...
        if ($ok) {
            ...
            $consumer->lti_version = $row['ltiVersion'];
            $consumer->consumer_name = $row['consumerName'];
            $consumer->consumer_version = $row['consumerVersion'];
            $consumer->consumer_guid = $row['consumerGuid'];
            $consumer->css_path = $row['css_path'];
            $consumer->protected = ($row['protected'] == 1);
            $consumer->enabled = ($row['enabled'] == 1);
            $consumer->enable_from = NULL;
            if (!is_null($row['enableFrom'])) {
                $consumer->enable_from = strtotime($row['enableFrom']);
            }
